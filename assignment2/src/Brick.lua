...
function Brick:hit(hasKey)
    ...
    if self.hasKey and self.isKey or not self.isKey then
...
function Brick:render()
    if self.inPlay then
        ...
        if self.isKey then
            love.graphics.draw(gTextures['main'], love.graphics.newQuad(161, 49, 31, 15, gTextures['main']:getDimensions()), self.x, self.y)
        else
